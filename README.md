...
- 提示词模板完全可自定义：System 与各场景 User 提示词均可修改，支持 19 个英文变量（角色名、时间、地图、对话记录、报纸、菜品、羁绊、记忆等），配置工具内可视化编辑并带变量速查复制
- 羁绊提示词：可按羁绊 1~5 级为每个稀客配置专属语气提示（支持全局兜底），经 {bondTone} 变量注入提示词模板，默认关闭可在通用设置开启
- 长期记忆：NPC 会记住与玩家的过往对话（跨天/跨存档生效），下次见面时自然承接之前聊过的话题；记忆按角色独立存储于 memories.json，每个角色最多保留 N 条（默认 10，超出自动丢弃最旧的），可在配置中开关、调整保留条数与每次注入条数
...
- 生成温度可在配置中滑块调节，默认 0.8
- 剧情对话、羁绊升级对话保持游戏原文，不做替换
- 生成超时或出错时自动回退游戏原文
- 长期记忆：对话结束时自动把本次对话存入 memories.json，生成台词时注入最近几条作为「过去的对话记忆」，让 NPC 自然提及上次聊过的话题；记忆按条数保留（每角色默认 10 条，超出丢最旧），不按天数过期；模板中可用 {memories} 变量自定义记忆段位置（未放置时自动追加到提示词末尾）
...
1. 下载 BepInEx 6 IL2CPP 版（[BepInEx 官方构建下载页](https://builds.bepinex.dev/projects/bepinex_be)，大多数情况下选择`BepInEx-Unity.IL2CPP-win-x64`[下载直链](https://builds.bepinex.dev/projects/bepinex_be/785/BepInEx-Unity.IL2CPP-win-x64-6.0.0-be.785%2B6abdba4.zip)）压缩包，不是此系统版本自行判断），解压到游戏根目录，运行一次游戏让其完成初始化
2. 从 [Releases](https://github.com/shrimp9527/MystiaAI/releases) 下载 `MystiaAI.dll`（[下载直链](https://github.com/shrimp9527/MystiaAI/releases/latest/download/MystiaAI.dll)），放入 `游戏根目录/BepInEx/plugins/`
...
- `settings.json`：开关、供应商、API Key、模型、字数上限、超时、温度、报纸话题频率与关键词触发开关、长期记忆开关与条数等
- `personas.json`：全部角色人设与 1~5 级羁绊提示词（角色专属 + 全局兜底），按中文名组织，可逐角色编辑（Label/ID 由游戏自动回填）
- `prompts.json`：System/User 提示词模板，支持占位符替换，留空字段回退内置默认
- `memories.json`：长期记忆数据（按角色分组，每角色最多保留 `memoryMaxPerCharacter` 条、超出丢最旧；自动生成，一般无需手动修改；游戏运行中编辑会在下次写入时被覆盖）
