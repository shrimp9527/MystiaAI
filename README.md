...
- 长期记忆：NPC 会记住与玩家的过往对话（跨天/跨存档生效），下次见面时自然承接之前聊过的话题；记忆按角色独立存储于 memories.json，可在配置中开关、调整保留条数与每次注入条数
...
- 生成超时或出错时自动回退游戏原文
- 长期记忆：对话结束时自动把本次对话存入 memories.json，生成台词时注入最近几条作为「过去的对话记忆」，让 NPC 自然提及上次聊过的话题；模板中可用 {memories} 变量自定义记忆段位置（未放置时自动追加到提示词末尾）
...
- `memories.json`：长期记忆数据（按角色分组，自动生成，一般无需手动修改；游戏运行中编辑会在下次写入时被覆盖）
